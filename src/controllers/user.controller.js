...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const { fullName, email, username, password } = req.body; // for form and json data

  // validation - not empty [using some method with callback functions] [.body by express]
  if (
  ...
  // check for images, check for avatar [.files by multer]
  ...
  // remove password and refresh token field from response [- : not include]
  ...
  if (!createdUser) {
    throw new Error(500, "Something went erong while registering the user");
  }
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // find the user - mongoDB aggregation
